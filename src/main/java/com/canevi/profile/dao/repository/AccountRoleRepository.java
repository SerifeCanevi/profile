...
public interface AccountRoleRepository extends JpaRepository<AccountRole, Long> {
    @Query("SELECT a FROM AccountRole a WHERE a.accountId = :accountId")
    Optional<List<AccountRole>> findByAccountId(Long accountId);

    @Query("SELECT COUNT(a) > 0 FROM AccountRole a WHERE a.accountId = :accountId AND a.role = :role")
    boolean findIfAccountHasRole(Long accountId, String role);

    @Query("DELETE FROM AccountRole a WHERE a.accountId = :accountId AND a.role = :role")
